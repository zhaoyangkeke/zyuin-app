  ...
  <div id="kgsqd" style="height: 100%" v-loading="loading">
    ...
    <div class="conter_table" v-if="!showDetail&&!showAdd">
      ...
      <div class="chaxun">
        <el-form :inline="true" style="display: inline-block;" class="searchForm">
          ...
          <el-form-item style="margin-bottom: 0px" label="" prop="" class="">
            <el-button @click="searchYDJH" size="mini"
      ...
      <el-table :data="YDJHData" tooltip-effect="dark"
        ...
        <!-- <el-table-column align="center" v-for="(tit,titIndex) in tableTits" :prop="titIndex" :label="tit" :key="titIndex" sortable show-overflow-tooltip> </el-table-column> -->
        ...
        <el-table-column align="left" prop="jcr" label="检查人" sortable show-overflow-tooltip></el-table-column>
        <el-table-column align="left" prop="htje" label="项目金额(元)" width="120" sortable
                         show-overflow-tooltip></el-table-column>
        <el-table-column align="left" prop="wyfj" label="违约金合计(元)" width="130" sortable
                         show-overflow-tooltip></el-table-column>
        <el-table-column align="left" prop="jcrq" label="检查日期" sortable show-overflow-tooltip></el-table-column>
        <el-table-column align="left" class-name="column-caoz" label="操作" width="100">
          <template scope="scope">
            <span v-if="scope.row.state !=1"
                  style="color:#00a2fd;cursor: pointer;text-align: center;margin-right: 10px;"
    ...
    <div class="tabArea" v-if="showAdd">
      <div style="position:relative;">
        <p class="tabAreaTit">
          <span class="titleclasschild" @click="addReturn(true)"><i
      ...
      <div>
        <div style="padding-left: 15px;">
          <el-form :model="khbModel" :rules="jldModelRules" ref="khbModel" label-width="150px">
            ...
            <el-form-item class="addJLD" style="width: 30%;" label="违约罚金合计(元)">
              <el-input :disabled="true" size="mini" v-model="khbModel.wyfj"></el-input>
            </el-form-item>
        ...
        <el-table :data="addData" tooltip-effect="dark"
          ...
          <el-table-column align="left" label="工程考核情况">
            <template slot-scope="scope">
              <el-input style="padding-left: 8px" size="mini"
                        v-model="addData[scope.$index].jcqk"></el-input>
            </template>
          </el-table-column>
          <el-table-column align="left" label="罚金(元)" show-overflow-tooltip>
            <template slot-scope="scope">
              <input style="padding-left: 8px;" type="number" size="mini" @keyup="changeFj"
                        v-model.number="addData[scope.$index].fj"/>
            </template>
          </el-table-column>
        </el-table>
      </div>
    ...
    <div class="tabArea" v-show="showDetail">
      <div style="position:relative;">
        <p class="tabAreaTit">
          <span class="titleclasschild" @click="promiseTG('close')"><i
            class="icon iconfont icon-arrow-left titleclasschild" style="cursor: pointer"></i></span>
          <span>进场准备情况检查表</span>
        </p>
      ...
      <div style="padding-right: 20px;margin-top: 20px;float: right;">
  ...
  export default {
    ...
    data() {
      return {
        ...
        searchInputVal: '',
        tableTits: {
          htbh: '项目编号', jcrq: '检查日期', htmc: '项目名称', sgdw: '施工单位',
          gldw: '管理单位', jcr: '检查人', htje: '合同金额', wyfj: '违约金合计', bz: '备注'
        },
        currentMonthData: [],
        addData: [],
    ...
    methods: {
      ...
      saveAdd() {
        ...
        let {
          htbh, htmc, jcrq, sgdw, jcr, htje, fj, bz, wyfj
        } = this.khbModel
        let arr = []
        ...
        if (htmc) {
          this.$api.zxaddJczb(params).then(res => {
            if (res.code === 1) {
              this.$message({
                message: '添加信息成功！',
                type: 'success'
      ...
      saveEdit() {
        let {bz, fj, htbh, htje, htmc, jcr, jcrq, sgdw, id, wyfj} = this.khbModel
        let self = this
        let editParams = {
          bmbh: undefined,
          bmid: undefined,
        ...
        this.$api.zxupdateJczb(editParams).then(res => {
          if (res.code === 1) {
            this.showAdd = false
            this.$message({
              message: '更新信息成功!',
      ...
      // 项目名称下拉列表
      getXmmcJcarr() {
        this.$api.getXmmcJcList().then(res => {
          if (res.status !== 200) {
            this.$message({
              message: '网络出错',
              type: 'error'
        ...
        if (bol) {
          ...
          this.$confirm('此操作将永久删除该文件, 是否继续?', {
            ...
            if (this.delIds.length) {
              this.$api.zxdeleteJczb(obj).then(res => {
                if (res.code == 1) {
                  this.$message({
                    message: '删除信息成功！',
                    type: 'success'
      ...
      //初始化列表数据
      queryYDJHData() {
        let {bmid, gldwid, yhid, userType} = this.userInfo
