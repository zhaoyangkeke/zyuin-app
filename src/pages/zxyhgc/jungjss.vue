  ...
  export default {
    ...
    methods: {
      ...
      showBhFormDia (row) {
        ...
        this.$api.getJungDetailed2({id:row.id}).then(res=>{
          if(res.code == 1){
            if (res.data) {
      ...
      search (currentPage, pageSize) {
        ...
        this.$api.getJungList3({pageNum:currentPage,pageSize,daZzid,daBmid,daYhid,xmbh,startDate:ksrq,endDate:jsrq}).then(resp => {
      ...
      addSubmit () {
        ...
        if(this.lmshForm.xmmc){
          this.$api.insertJung2(this.lmshForm).then(res => {
      ...
      updateSubmit () {
        ...
        this.$api.updateJung2(this.lmshForm).then(res => {
          if (res.code === 1) {
            this.$message({
              type: 'success',
              message: '编辑成功!'
      ...
      deleteData () {
        ...
        if (this.multipleSelection.length > 0) {
          this.$confirm('确定删除选中的信息吗？删除后数据无法恢复！', '', {
            ...
            this.$api.deleteJung2({id:djbharr}).then(res => {
              if (res.code === 1) {
                this.$message({
                  type: 'success',
                  message: '删除成功!'
      ...
      shenhe (row, index) {
        ...
        this.$confirm('是否通过审核？', '', {
          ...
          this.$api.createWorkFlow({
            ...
            org:orgmc}).then(resp=>{
            if(resp.code == 1){
              this.$api.updateJungSpzt2({id:row.id,spzt:"4002"}).then(res => {
                if (res.code === 1) {
                  this.$message({
                    type: 'success',
                    message: '审核通过!'
      ...
      selectHt (value) {
        ...
        this.$api.getJungBasicData2({xmbh:value.XMBH}).then(res=>{
          if(res.code == 1){
