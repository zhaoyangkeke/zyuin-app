...
*/
<template>
  <div id="wxjs" class="styleSheets" style="height: 100%" v-loading.fullscreen.lock="fullscreenLoading"  v-if="isRouterAlive">
<div class="bill-header">
    ...
    <div class="conter_table">
      <el-table   ref="multipleTable" :data="tableData" tooltip-effect="dark"
        ...
        <!-- <el-table-column align="center" prop="wxdh" label="单据编号" show-overflow-tooltip></el-table-column> -->
        ...
        <el-table-column sortable align="left" prop="jsje" label="结算金额(元)" show-overflow-tooltip></el-table-column>
        <el-table-column sortable align="left" prop="wxsm" label="维修说明" show-overflow-tooltip></el-table-column>
    ...
    <el-table-column
      ...
      <el-form v-if="!showxq" :model="lmshForm" :rules="wxjs" ref="lmshForm" label-width="100px" class="demo-lmshForm">
                  <el-form-item class=" addJLD" label="维修单号" prop="wxdh">
        
              <el-input  size="mini" type="text" v-model="lmshForm.wxdh" @focus="showDialog('wx')" placeholder="请选择维修单号"></el-input>
              </el-form-item>
              <el-form-item class=" addJLD" label="仓库名称" prop="ckmc">
                <span class="edit_text">{{lmshForm.ckmc}}</span>
              </el-form-item>
                <el-form-item class=" addJLD" label="维修方式" prop="wxfs">
              <span class="edit_text">{{lmshForm.wxfs}}</span>
              </el-form-item>
                <el-form-item class=" addJLD" label="结算金额(元)" prop="jsje">
                <span class="edit_text">{{lmshForm.jsje}}</span>
              </el-form-item>
              
              <el-form-item class=" addJLD_long" label="维修说明" prop="wxsm">
                <el-input type="textarea" :autosize="{ minRows: 2, maxRows: 4}" placeholder="请输入内容" v-model="lmshForm.wxsm"></el-input>
              </el-form-item>
         ...
         <div class="group-form-name hide-form-item"><div class="operator"  @click="showZb=!showZb"><span class="name">结算清单</span><a :class="showZb?'el-icon-minus':'el-icon-plus'" href="javascript:;"   style="color: #999;font-size:12px;text-decoration: none;font-weight:900"></a></div><span class="line"></span></div>
        ...
        <div class="zibnr" v-if="addzb" style="overflow-x: auto;overflow-y: hidden;">
          ...
          <ul>
            <li>
             <span>序号</span><span>维修资产</span><span>维修费用(元)</span><span>其他费用(元)</span><span>发票号</span><span>税率</span><span>税额(元)</span><span>备注</span>
            </li>
            <li v-for="(item, index) in lmshForm.mx" :key="index">
              ...
              <p>
                <el-input disabled size="mini" type="text" v-model="item.wxbj"></el-input>
              </p>
              <p>
      ...
      <!-- <div class="dialog_warps">
        <ul class="zhubnr">
          <li>
            <span>维修单号</span>
            <p>
              <el-select clearable v-if="!showxq" size="mini" style="width: 148px;" v-model="lmshForm.wxdh" value-key="DJBH" placeholder="" @change="selectDJBH">
                <el-option v-for="item in ckOptions" :key="item.DJBH" :label="item.DJBH" :value="item"></el-option>
              </el-select>
          ...
          <li>
            <span>仓库名称</span>
            <p>
              <el-input size="mini" type="text" v-model="lmshForm.ckmc"></el-input>
            </p>
          </li>
          <li>
            <span>维修方式</span>
            <p>
              <el-input size="mini" type="text" v-model="lmshForm.wxfs"></el-input>
            </p>
          </li>
          <li>
            <span>结算金额(元)</span>
          ...
          <li class="text_line">
            <span>维修说明</span>
            <p>
              <el-input size="mini" type="textarea" v-model="lmshForm.wxsm"></el-input>
            </p>
          </li>
           <li class="text_line">
            <span>附件</span>
          ...
          <li class="text_line" v-if="lmshForm.wxfs==='返厂维修'">
            <span>维修厂商</span>
            <p>
              <el-input size="mini" type="textarea" v-model="lmshForm.wxcs"></el-input>
            </p>
          </li>
          <li class="text_line">
            <span>备注</span>
        ...
        <div class="zibnr" v-if="addzb" style="overflow-x: auto;overflow-y: hidden;">
          ...
          <ul>
            <li>
             <span>序号</span><span>维修备件</span><span>维修费用</span><span>其他费用</span><span>发票号</span><span>税率</span><span>税额</span><span>备注</span>
            </li>
            <li v-for="(item, index) in lmshForm.mx" :key="index">
              <p>
                <el-input size="mini" type="text" :value="index+1"></el-input>
              ...
              <p>
                <el-input size="mini" type="text" v-model="item.wxbj"></el-input>
              </p>
              <p>
                <el-input size="mini" type="text" v-model="item.wxfy" @input="hjwxfy"></el-input>
              </p>
...
export default {
  ...
  data () {
    return {
      ...
      restaurantsBH:[ ],
      datatypes:'',
      wxfs:[{mjmc:'自行维修',mjbm:0},{mjmc:'返厂维修',mjbm:1}],
       dialogTableTit:{
         ckmc:'仓库名称',wxfs:'维修方式',tbr:'填报人',tbdw:'填报单位',tbbm:'填报部门',tbsj:'填报时间'
        },
        totals: 0,
      dialogData:[],
      dialogVisible:false,
 ...
 async created(){
  ...
  watch: {
    multipleSelection(data) {  //存储选中的row
     ...
     next(vm=>{
       vm.$api.jdwhbjglgetFjwxdInfByDw().then(resp => {
        if (resp.status !== 200) {
          vm.$message({
            message: '出错了',
            type: 'error'
...
if(n[ n.length - 1 ] == "%"){
    ...
    if (sea.jcsj) {
        ...
        sea.jsrq = ''
         ...
         this.$api.jdwhbjglgetFjwxList('?pageNum=' + val.currentPage + '&pageSize=' + val.pageSize+ '&' + $.param(sea)).then(resp => {
        this.loading = false
        if (resp.status !== 200) {
          this.$message({
            message: '出错了',
      ...
      showDialog(bol){
        this.dialogVisible = true
         this.datatypes = bol
      this.$api.jdwhbjglgetFjwxList('?pageNum=' + 1 + '&pageSize=' + 10 +'&tbbmdm=' + this.userInfo.daBmbm+'&tbdwdm=' +this.userInfo.daZzbh).then(resp => {
        this.loading = false
        if (resp.status !== 200) {
          this.$message({
            message: '出错了',
      ...
      closeDialog(val){
        ...
        this.lmshForm.mx=[]
          ...
          this.lmshForm.wxfs=val.wxfs
          this.lmshForm.ckmc=val.ckmc 
        console.log(val)
        if(val){
         this.$api.jdwhbjglgetFjwxdBjInfByDw('?djbh=' + val.djbh).then(res => {
...
this.searchForm.tbbmdm=this.userInfo.daBmbm
    ...
    showAdd () {
      ...
      this.showxq = false
      this.isadd = true
      this.formId = 'wxjs' + new Date().getTime()
    ...
    updateFormDia (row, index) {
      ...
      this.$api.getFilesDataById('?id='+this.lmshForm.wxdh).then(res=>{
       if(res.data.code==1){
    ...
    search (currentPage, pageSize) {
      ...
      this.$api.jdwhbjglgetFjwxjsList('?pageNum=' + currentPage + '&pageSize=' + pageSize + '&' + $.param(this.searchForm)).then(resp => {
        this.loading = false
        if (resp.status !== 200) {
          this.$message({
            message: '出错了',
    ...
    for (let index = 0; index < this.lmshForm.mx.length; index++) {
      if(this.lmshForm.mx[index].wxfy==''){
        this.lmshForm.mx[index].wxfy=0
      }
      if(this.lmshForm.mx[index].qtfy==''){
        this.lmshForm.mx[index].qtfy=0
      }
    if(isNaN(this.lmshForm.mx[index].wxfy) || this.lmshForm.mx[index].wxfy<0){
        this.$message({
            type: 'error',
            message: '维修费用只能是大于等于0的数字!'
          })
          return
      }
      if(isNaN(this.lmshForm.mx[index].qtfy) || this.lmshForm.mx[index].qtfy<0){
        this.$message({
            type: 'error',
            message: '其他费用只能是大于等于0的数字!'
          })
          return
      }      
      }
      ...
      for (let index = 0; index < this.dataList.length; index++) {
        this.dataList[index].file_id=this.lmshForm.wxdh
      }
      ...
      this.$api.jdwhbjgladdFjwxjs(this.lmshForm).then(res => {
        if (res.data.code === 1) {
          this.$message({
            type: 'success',
            message: '添加成功!'
      ...
      this.$api.jdwhbjglgetFjwxdInfByDw().then(resp => {
        if (resp.status !== 200) {
          this.$message({
            message: '出错了',
            type: 'error'
    ...
    if(isNaN(this.lmshForm.mx[index].wxfy) || this.lmshForm.mx[index].wxfy<0){
      ...
      this.$api.jdwhbjgleditFjwxjs(this.lmshForm).then(res => {
        if (res.data.code === 1) {
          this.$message({
            type: 'success',
            message: '编辑成功!'
    ...
    deleteData () {
      if (this.multipleSelection.length > 0) {
        this.$confirm('确定删除选中的信息吗？删除后数据无法恢复！', '', {
          ...
          this.$api.jdwhbjgldelFjwxjsByids('?ids=' + djbharr).then(res => {
            if (res.data.code === 1) {
              this.$message({
                type: 'success',
                message: '删除成功!'
    ...
    selectDJBH (value) {
      this.lmshForm.ckmc = value.CKMC
      this.lmshForm.wxfs = value.WXFS
      this.lmshForm.wxcs = value.WXCS
      this.lmshForm.wxdh = value.DJBH
      this.lmshForm.jsje = 0
      this.$api.jdwhbjglgetFjwxdBjInfByDw('?djbh=' + value.DJBH).then(res => {
        this.lmshForm.mx = res.data.data
  ...
  mounted () {
    ...
    this.getQmldbb()
    // this.tableHeight = $('#wxjs')[0].offsetHeight
