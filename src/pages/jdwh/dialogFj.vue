      ...
      <div >
          <el-form  :inline="true" style="padding-left:16px">
               ...
               <el-form-item class="" label="" prop="">
                <el-input clearable size="mini" type="text"  v-model="searchVal.bjmc" placeholder="请输入备件名称" class="space-blank">
			  </el-input>
                 </el-form-item>   
                <!-- <el-form-item class="" label="" prop="">
                <el-select clearable size="mini" v-model="ckmcs" placeholder="请选择仓库" value-key="ssck">
          <el-option
       ...
       <div style="height:100%;display: flex;flex-wrap: nowrap;padding-left:16px;padding-right:16px;overflow: hidden;">
...
<script>
  export default {
    props:['ckmc','dialogVisibleWl','datatypesWl','dialogTableTitWl','btWl','dwbm','bmbm','mx'],
    data(){
      return {
        searchVal:{userName:''},
        searchType:'',
    ...
    this.loading=true
      this.$api.getShFjList('?pageNum=' + currentPage + '&pageSize=' + pageSize +'&ckmc='+this.ckmc + '&tbdwdm='+ this.dwbm+ '&'+ $.param(this.searchVal)).then(resp => {
        this.loading = false
        if (resp.status !== 200) {
          this.$message({
            message: '出错了',
        ...
        for(let j=0;j<this.dialogDataWl.length;j++){
         ...
         if(this.mx[i].bjmc==this.dialogDataWl[j].sbmc && this.mx[i].bjbh==this.dialogDataWl[j].sbbm && this.mx[i].ggxh==this.dialogDataWl[j].ggxh && this.mx[i].jldw==this.dialogDataWl[j].jldw){
         ...
         for(let j=0;j<this.dialogDataWl.length;j++){
          ...
          if(Number(this.dialogDataWl[j].kcsl) <=0){
            this.dialogDataWl.splice(j,1)
          }
          }
