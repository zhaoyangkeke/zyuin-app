      ...
      <div >
          <el-form  :inline="true" style="padding-left:16px">
               ...
               <el-form-item class="" label="" prop="">
                <el-input clearable size="mini" type="text"  v-model="searchVal.lxbh" placeholder="请输入立项编号" class="space-blank">
			  </el-input>
                 </el-form-item>   
                <el-form-item class="" label="" prop="">
               <el-input clearable size="mini" type="text"  v-model="searchVal.xmlxmc" placeholder="请输入项目类型名称">
			  </el-input>
                 </el-form-item>   
            <el-form-item class="" label="" prop="">
       ...
       <div style="height:100%;display: flex;flex-wrap: nowrap;padding-left:16px;overflow: hidden;">
        ...
        <el-table-column
          ...
          <el-table-column align="center"  style="background: #afc;" v-for="(tit,titIndex) in dialogTableTitSp" :prop="titIndex" :label="tit" :key="titIndex"> </el-table-column>
        </el-table>
           </div>
...
<script>
  export default {
    props:['dialogVisibleSp','datatypesSp','dialogTableTitSp','btSp','bmbm','dwbm'],
    data(){
      return {
        searchVal:{userName:''},
        searchType:'',
    ...
    watch: {
      dialogVisibleSp:function (newV,oldV) {
      },
    multipleSelection(data) {  //存储选中的row
      this.selectRow = [];
      if (data.length > 0) {
        data.forEach((item, index) => {
          this.selectRow.push(item.lxbh);
        });
      }
    }
  },
    created (){
        this.istype=this.datatypesSp
    ...
    this.loading=true
      this.$api.getLxspList('?pageNum=' + currentPage + '&pageSize=' +pageSize+ '&dwbm='+this.dwbm+'&bmbm='+''+'&'+$.param(this.searchVal)).then(resp => {
        if (resp.status !== 200) {
          this.$message({
            message: '出错了',
            type: 'error'
    ...
    rowClass({ row, rowIndex }) {
      ...
      if (this.selectRow.includes(row.lxbh)) {
   ...
   this.searchZc(this.currentPage, this.pageSize)
      ...
      conformClose(type){
             ...
             this.$emit('closeDialogSp',this.selectSearch)
