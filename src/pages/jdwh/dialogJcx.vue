    ...
    fullscreen class="data-dialog el-updata-div  jqk" 
      ...
      <div >
       ...
       <div style="height:100%;display: flex;flex-wrap: nowrap;padding-left:16px;overflow: hidden;">
           ...
           <div style="display:inline-block;width:100%;height:100%;border-left: 1px solid #edeef0;vertical-align: top;border-right: 1px solid #edeef0">
...
<script>
  export default {
    ...
    created (){
      console.log(this.mx)
      this.searchVal.ssdwid=this.dwbm
    ...
    this.loading=true
    console.log(this.mx)
      this.$api.getXclxmxjList('?pageNum=' + currentPage + '&pageSize=' + pageSize + '&'+ $.param(this.searchVal)).then(resp => {
        this.loading = false
        if (resp.status !== 200) {
          this.$message({
            message: '出错了',
        ...
        for (let i = 0; i < this.mx.length; i++) {
         if(this.mx[i].mx_jcx=='' || this.mx[i].mx_jcx==undefined){
            this.mx.splice(i,1)
          }
        }
        for(let i=0;i<this.mx.length;i++){
        for(let j=0;j<this.dialogDataWl.length;j++){
         if(this.mx[i].mx_jcx==this.dialogDataWl[j].jcx){
