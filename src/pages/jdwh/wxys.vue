...
*/
<template>
  <div id="wxys" class="styleSheets" style="height: 100%" v-loading.fullscreen.lock="fullscreenLoading"  v-if="isRouterAlive">
<div class="bill-header">
      ...
      <div class="bill-search" style="">
        <el-form :inline="true" :model="searchForm" ref="searchForm" class="searchForm">
          <el-form-item style="margin-bottom: 0px" prop="" class="">
            ...
            <el-input clearable class="selectDw"  style="width:200px"  v-model="selectDw" size="mini" placeholder="请选择填报部门" @focus="selectDwTree" @change="clears"><i
    class="el-icon-error"
    slot="suffix"
    ...
    <div class="conter_table">
      <el-table   ref="multipleTable" :data="tableData" tooltip-effect="dark"
        ...
        <el-table-column sortable align="center" class-name="column-caoz" label="状态" width="80">
          <template scope="scope">
            <span style="color:#D34C42;cursor: pointer;text-align: center" v-if="scope.row.yszt==='已验收'">已验收</span>
            <span style="color:#454C53;text-align: center" v-if="scope.row.yszt!='已验收'">未验收</span>
          </template>
        </el-table-column>
        <!-- <el-table-column align="center" prop="djbh" label="单据编号"  show-overflow-tooltip></el-table-column> -->
        <!-- <el-table-column align="left" prop="bxdh" label="报修单号" show-overflow-tooltip></el-table-column> -->
        ...
        <el-table-column sortable width="100"  align="left" prop="gzdj" label="故障等级" show-overflow-tooltip></el-table-column>
        <el-table-column  width="100"  sortable align="left" prop="bzrq" label="报障日期"></el-table-column>
        ...
        <!-- <el-table-column width="100"  sortable align="left" prop="qrsj" label="确认时间"></el-table-column> -->
        <el-table-column  align="center" class-name="column-caoz" label="操作" width="80">
          <template scope="scope">
            <span v-if="scope.row.ys && scope.row.yszt!='已验收'" style="color:#00a2fd;cursor: pointer;text-align: center" @click="updateFormDia(scope.row, scope.$index)">验收</span>
    ...
    <el-dialog fullscreen class="data-dialog el-updata-div" title="维修验收" :close-on-click-modal="false" :visible.sync="showDetail" width="580px" :show-close="false">
      <span slot="title">
      ...
      <div class="group-form-name hide-form-item"><div class="operator"  ><span class="name">验收人意见表</span><a :class="showZb?'el-icon-minus':'el-icon-plus'" href="javascript:;"   style="color: #999;font-size:12px;text-decoration: none;font-weight:900"></a></div><span class="line"></span></div>
              <el-table
    :data="lmshForm.yslist"
    border
    style="width: 100%;">
    <el-table-column
      type="index"
    ...
    <el-table-column
     width="100"
      ...
      <el-form v-if="!showxq" :model="lmshForm" :rules="wxys" ref="lmshForm" label-width="100px" class="demo-lmshForm">
          <el-form-item class=" addJLD" label="维修结果确认" prop="wxjgqr">
              <el-radio v-model="lmshForm.wxjgqr" label="通过" >通过</el-radio>
              <el-radio v-model="lmshForm.wxjgqr" label="不通过" >不通过</el-radio>
              </el-form-item>
              <el-form-item class=" addJLD" label="验收时间" prop="yssj">
                 <el-date-picker
                v-model="lmshForm.yssj"
                size="mini"
                type="date"
                format="yyyy-MM-dd"
                value-format="yyyy-MM-dd"
              ...
              <el-form-item   class=" addJLD_long" label="验收人意见" prop="ysryj">
                 <el-input size="mini" type="textarea" v-model="lmshForm.ysryj"></el-input>
              </el-form-item>
    ...
    <el-table-column
     width="100"
      ...
      label="验收时间">
              ...
              <div class="group-form-name hide-form-item"><div class="operator"  @click="showZb=!showZb"><span class="name">详细信息</span><a :class="showZb?'el-icon-minus':'el-icon-plus'" href="javascript:;"   style="color: #999;font-size:12px;text-decoration: none;font-weight:900"></a></div><span class="line"></span></div>
              <div v-if="showZb"> 
             ...
             <el-form-item class=" addJLD" label="设备名称" prop="sbmc">
              ...
              <el-form-item class=" addJLD" label="维修日期" prop="wxrq">
               <span class="edit_text">{{lmshForm.wxrq}}</span>
              </el-form-item>
              <el-form-item class=" addJLD" label="现场维修人" prop="xcwxr">
                <span class="edit_text">{{lmshForm.xcwxr}}</span>
              </el-form-item>
              <el-form-item  v-if="lmshForm.isww==='是'"  class=" addJLD" label="施工方负责人" prop="sgffzr">
               <span class="edit_text">{{lmshForm.sgffzr}}</span>
              </el-form-item>
              <el-form-item class=" addJLD" label="确认人" prop="qrr">
                <span class="edit_text">{{lmshForm.qrr}}</span>
              </el-form-item>
    ...
    return {
      ...
      wxys:{
        yssj: [
            ...
            { min: 1, message: '请填写确认时间'}
          ], 
        ysryj:[
            { required: true, message: '验收人意见不能为空',trigger: 'blur'},
            { min: 1, message: '请填写验收人意见'}
          ], 
      },
      lmshForm: {
        ysryj:'',
...
this.searchForm.tbbmdm=this.userInfo.daBmbm
    ...
    updateFormDia (row, index) {
      ...
      let Arr= row.ysrcode.split(',')
 
      let flag=false
      for (let index = 0; index < Arr.length; index++) {
       if(this.userInfo.daYhbm ==Arr[index]){
        flag=true
       }     
      }
      if(flag==false){
        this.$message({
              message: '你不是验收人没有权限编辑',
              type: 'error'
            })
        return
      }
      ...
      this.showxq = false
      this.index = index
      this.$api.jdwhxjwxgetInfoByIDjbh('?djbh=' + row.bxdh).then(resp => {
          if (resp.status !== 200) {
            this.$message({
              message: '出错了',
              type: 'error'
            ...
            this.lmshForm.yssj= this.formatDate(new Date())
            for (let index = 0; index < this.lmshForm.yslist.length; index++) {
              if(this.userInfo.daYhbm== this.lmshForm.yslist[index].ysrcode){
                this.lmshForm.ysryj=this.lmshForm.yslist[index].ysryj
              }
    ...
    showBhFormDia (row) {
      this.showDetail = true
      this.showxq = true
      for (let index = 0; index < row.mx.length; index++) {
              if(row.qrr== row.mx[index].ysr){
                row.ysryj=row.mx[index].ysryj
              }
    ...
    search (currentPage, pageSize) {
      ...
      this.$api.jdwhxjwxgetWxysList('?pageNum=' + currentPage + '&pageSize=' + pageSize + '&' + $.param(this.searchForm)).then(resp => {
        this.loading = false
        if (resp.status !== 200) {
          this.$message({
            message: '出错了',
          ...
          var reg= new RegExp(this.userInfo.daYhbm)
          for (let index = 0; index < this.tableData .length; index++) {
        this.tableData[index].ys=false 
          }
          for (let index = 0; index < this.tableData.length; index++) {
         if(reg.test(this.tableData[index].ysrcode)){
              this.tableData[index].ys=true 
          }     
      }
    ...
    updateSubmit () {
      ...
      if(this.lmshForm.yslist.length==0){
          let obj={
            ysr:this.userInfo.daYhmc,
            ysryj:this.lmshForm.ysryj,
            ysrcode:this.userInfo.daYhbm,
            yszt:this.lmshForm.wxjgqr,
            yssj:this.lmshForm.yssj,
            ysbxdh:this.lmshForm.djbh
          }
         this.lmshForm.yslist.push(obj) 
      }else{
        let fg=false
     for (let i = 0; i < this.lmshForm.yslist.length; i++) {
         if(this.userInfo.daYhbm== this.lmshForm.yslist[i].ysrcode){
           fg=true
           this.lmshForm.yslist[i].ysr=this.userInfo.daYhmc
           this.lmshForm.yslist[i].ysrcode=this.userInfo.daYhbm
          this.lmshForm.yslist[i].ysryj=this.lmshForm.ysryj
          this.lmshForm.yslist[i].yszt=this.lmshForm.wxjgqr
           this.lmshForm.yslist[i].yssj=this.lmshForm.yssj
           this.lmshForm.yslist[i].ysbxdh=this.lmshForm.djbh
         }
    ...
    this.lmshForm.ryid=config.userInfo.yhid
      this.$api.jdwhxjwxeditWxys(this.lmshForm).then(res => {
        if (res.data.code === 1) {
          this.$message({
            type: 'success',
            message: '验收成功!'
    ...
    deleteData () {
      if (this.multipleSelection.length > 0) {
        this.$confirm('确定删除选中的信息吗？删除后数据无法恢复！', '', {
          ...
          this.$api.jdwhxjwxdelWxysByDjbhs('?djbhs=' + djbharr).then(res => {
            if (res.data.code === 1) {
              this.$message({
                type: 'success',
                message: '删除成功!'
    ...
    shenhe (row, index) {
      this.$confirm('是否通过审核？', '', {
        ...
        this.$api.jdwhxjwxspWxysByDjbh('?spzt=已验收&djbhs=' + form.djbh).then(res => {
          if (res.data.code === 1) {
            this.$message({
              type: 'success',
              message: '验收通过!'
    ...
    selectHt (value) {
      ...
      this.lmshForm.wxrq = value.wxrq
      this.lmshForm.xcwxr = value.xcwxr
      this.lmshForm.wxjgms = value.wxjgms
    },
    getQmldbb () {
  ...
  mounted () {
    ...
    this.getQmldbb()
    // this.tableHeight = $('#wxys')[0].offsetHeight
    document.onclick=function(e){
      if(e.target&&e.target.className!='el-input__inner'){
         $('.tbdw').hide()
      }
    }
    // this.search(this.currentPage, this.pageSize)d
  }
}
</script>
