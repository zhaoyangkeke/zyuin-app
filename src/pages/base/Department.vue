<template>
  <HorizontalLayout lWidthOpen="200px">
    <div slot="left" class="layout-div dep-layout">
      <el-scrollbar  :noresize="false" :native="false" >
        <div class="dep-scrollbar">
          <TreeMenu :id="treeId" :zNodes="nodes" :setting="defaultSetting" @selectNode="selectNode"></TreeMenu>
        </div>
      </el-scrollbar>
    </div>
    <div class="layout-div" style="margin-left: 10px;">
      <div class="layout-div" style="width: calc(100% - 10px);">
...
export default {
  components: {VerticalLayout, HorizontalLayout, TreeMenu},
  data () {
    return {
  ...
  methods: {
    ...
    checkTreeNode (event, treeId, treeNode) {
      this.selectNode(treeNode)
    },
    clickTreeNode (event, treeId, treeNode) {
      $.fn.zTree.getZTreeObj(this.treeId).checkNode(treeNode, !treeNode.checked, true)
      this.selectNode(treeNode)
    },
    ...
    onSeach () {
      this.$api.treedata().then(resp => {
        if (resp.status !== 200) {
          this.$message({message: '出错了', type: 'error'})
        } else {
          this.nodes = resp.data.treedata
        }
      }).catch(e => {
      })
