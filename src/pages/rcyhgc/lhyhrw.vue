    ...
    <div style="position: absolute;bottom: 0;width: 100%;background: #fff;height: 280px">
      <div class="header">
        <div class="cha_xun" style="">
          <el-form :inline="true" :model="searchForm" ref="searchForm" class="searchForm">
            <el-form-item style="margin-bottom: 0px" label="项目名称" prop="" class="">
              <el-autocomplete
                class="inline-input"
                size="mini"
            ...
            <!--  <el-select clearable size="mini" style="width: 150px;margin-right: 50px" v-model="lxData" value-key="ldcode" placeholder="路线编号" @change="changelx">
                <el-option v-for="item in qmbbOptions" :key="item.ldcode" :label="item.ldname" :value="item"></el-option>
              </el-select>-->
            </el-form-item>
      ...
      <div class="conter_table">
        <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark"
          ...
          <el-table-column align="left" sortable prop="lxcode" label="路线"  show-overflow-tooltip> </el-table-column>
          <el-table-column align="left" sortable prop="xcfx" label="方向" show-overflow-tooltip> </el-table-column>
          <el-table-column align="left" sortable prop="tbr" label="人员" show-overflow-tooltip> </el-table-column>
          <el-table-column align="left" sortable prop="ksrq" label="开始时间"> </el-table-column>
          <el-table-column align="left" sortable prop="jsrq" label="结束时间"> </el-table-column>
          <el-table-column align="left" sortable prop="qxxx" label="气象信息"> </el-table-column>
        </el-table>
      </div>
      <footer class="footer">
...
export default {
  ...
  methods: {
    ...
    resizeSearch(){
   this.lxData = undefined
   this.searchForm.xcfx = undefined
   this.searchForm.jcrq = undefined
    ...
    rowClick(row){
      ...
      self.gjlist = []
      let srr = []
        this.$api.getLhyhgj({id:row.id}).then(res=>{
          if(res.code == 1){
            let data = res.data
            if(data&&data){
              data.forEach(item=>{
    ...
    search (currentPage, pageSize, val) {
      ...
      this.$api.getLhyhrwList({
        pageNum:currentPage,pageSize,ksrq,jsrq,orgid:userInfo.orgid,yhid:userInfo.yhid,bmid:userInfo.bmid,xmbh: xmbh == undefined ? xmbhOfinit.XMBH : xmbh}).then(resp => {
        this.loading = false
        if (resp.status !== 200) {
          this.$message({
            message: '出错了',
