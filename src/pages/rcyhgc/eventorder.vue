...
*/
<template>
  <div id="kgsqd" style="height: 100%" class="container">
    <div class="header">
      <span class="headerTit">事件清单</span>
      <div class="header_option">
        <ul>
          <!--<li><i class="el-icon-search"> 查询</i></li>-->
          <!--<li><i class="icon iconfont icon-export"> 导出</i></li>-->
          <li @click="updateFormDia('add');isseedetail = 0"><i class="icon iconfont icon-edit_line"> 添加</i></li>
          <li @click="deleterows"><i class="icon iconfont icon-delete"> 删除</i></li>
        </ul>
      </div>
    </div>
    <!--删除区域弹框-->
    <!--<el-button type="text" @click="deleterows" v-if="isopendelete">点击打开11 Message Box</el-button>-->
    <!--查询弹框区域-->
    <div class="conter_table" style="padding: 0 15px;">
      <!--查询信息-->
      <div class="chaxun">
      ...
      <el-table  :data="YDJHData" tooltip-effect="dark"
        ...
        <el-table-column align="left" class-name="column-caoz" label="操作" width="290">
          <template scope="scope">
            ...
            <span   v-if="scope.row.zt > 2 ? false : true" style="color:#00a2fd;cursor: pointer;text-align: center;margin-right: 10px;"
                    @click="getrowId(scope.row);">指派</span>
            <span   style="color:#00a2fd;cursor: pointer;text-align: center;margin-right: 10px;"
    ...
    <el-dialog
      ...
      <div style="padding-bottom: 20px;text-align: center">
        <!--<span>指派给：</span>-->
        <span>点击确定系统将自动将任务指派给工程部</span>
        <div class="zpcontent">
          <!--<el-button @click="sendZpparam({id:rowobjId,zt:3})" size="mini" class="btn">确定</el-button>-->
          <!--<el-button @click="dialogVisible1 = false" size="mini" class="btn">取消</el-button>-->
        </div>
      </div>
    ...
    <div class="tabArea" v-show="showDetail">
      <div style="position:relative;">
        <p class="tabAreaTit">
          <span class="titleclasschild"  @click="showDetail = false;isxmmcOfadd = 0"><i class="icon iconfont icon-arrow-left titleclasschild" style="cursor: pointer"></i></span>
          <span>{{firstItemTit}}</span>
        </p>
        <div class="SPState">
          <el-button size="mini" class="minibtn"  v-if="isseedetail == 0 && isupdate == false"  @click="addjobOrder('jldModel');isxmmcOfadd = 0"
                     style="display:block;background: #DF4D4A; margin: auto; border-color:transparent;color: #fff;">保存</el-button>
          <el-button size="mini" class="minibtn"  v-if="isseedetail == 0 && isupdate == true" @click="submitjobOrder('jldModel');isxmmcOfadd = 0"
                     style="display:block;background: #DF4D4A; margin: auto; border-color:transparent;color: #fff;">保存</el-button>
        </div>
      </div>
      <div v-show="isseedetail == 0">
        <div style="padding-left: 15px;">
          <el-form style="font-size: 0"   :model="jldModel" :rules="jldModelRules" ref="jldModel" class="taskorder" label-width="100px">
            ...
            <el-form-item class="addJLD" label="路线名称">
              <el-input size="mini" v-model="jldModel.yhdw"  placeholder="请输入内容" disabled></el-input>
            </el-form-item>
            ...
            <el-form-item class="addJLD" label="管理单位" prop="gldw">
            ...
            <el-form-item class="addJLD" label="日期" prop="fxsj">
              <el-date-picker
                size="mini"
        ...
        <div style="padding-left: 15px;">
          <el-form :model="jldModel" style="font-size: 0" :rules="jldModelRules" ref="jldModel" label-width="100px">
            ...
            <el-form-item class="addJLD" label="巡查方向" prop="xcfx">
              <el-select  size="mini" v-model="jldModel.xcfx" placeholder="请选择" @change="handlechangexcfx">
                <el-option
            ...
            <el-form-item class="addJLD" label="车道" prop="cd">
              <el-select size="mini" v-model="jldModel.cd" placeholder="请选择" @change="changeCd">
                <el-option v-for="(item, index) in cdarr" :key="index" :label="item.mjmc" :value="item.mjbm"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item class="addJLD" label="单位工程" prop="dwgc">
              <el-input size="mini"  v-model="jldModel.dwgc"  placeholder="请输入内容"></el-input>
            </el-form-item>
            <el-form-item class="addJLD" label="损坏类型" prop="shlx">
              <el-select size="mini" v-model="jldModel.shlx" placeholder="请选择" @change="handlechangeshlx">
                <el-option
            ...
            <el-form-item size="mini" class="addJLD" label="病害类型" prop="bhlx">
              <el-cascader
                size="mini"
                :options="bhlxarr"
                :props="props"
            ...
            <el-form-item class="addJLD" label="病害数量" prop="bhsl">
              <el-input size="mini"  v-model.number="jldModel.bhsl"   placeholder="请输入内容"></el-input>
            </el-form-item>
            <el-form-item class="addJLD" label="病害单位" prop="bhdw">
              <el-input size="mini"  v-model="jldModel.bhdw" placeholder="请输入内容" disabled></el-input>
            </el-form-item>
            <el-form-item class="addJLD" label="工程量类型" prop="gcllx">
              <el-select size="mini" v-model="jldModel.gcllx" placeholder="请选择" @change="handlechangecllx">
                <el-option
            ...
            >
              <el-input size="mini"  v-model.number="jldModel.qzh"   placeholder="请输入内容" @blur="compareqdzh"></el-input>
            </el-form-item>
            ...
            >
              <el-input size="mini"  v-model.number="jldModel.zzh"   placeholder="请输入内容"></el-input>
            </el-form-item>
            <div class="bhsm">
      ...
      <div  v-show="isseedetail == 1" >
        ...
        <div style="padding-left: 15px; font-size: 0;">
          ...
          <p class="addJLD">
            ...
            <span class="spanwidth">{{jldModel.cd == 1 ? '单车道' : jldModel.cd == 2 ? '双车道' : jldModel.cd == 4 ? '四车道' : '六车道'}}</span>
          </p>
          <p class="addJLD">
...
<script>
  ...
  export default {
    ...
    data () {
      ...
      //   }
      // }
      // var qdzh1 = (rule, value, callback) => {
      //   if (val < this.qszh1 || val > this.zzzh2) {
      //     callback(new Error('起点桩号格式不正确'))
      //   } else {
      //     callback()
      //   }
    ...
    methods: {
      ...
      // 根据起止点桩号和路线编码get经纬度
      getLonLatByPos (param1,param2,param3) {
        ...
        this.$msgbox({
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              this.sendZpparam({id: rowId,zt: zt})
              done();
              setTimeout(() => {
      ...
      handlexmbh (val) {
        ...
        this.jldModel.xmbh = val
        this.getXmbhtomcArr(val)
        this.getZhList(val)
      },
      // change 项目名称
      handlechangexmmc (val) {
        this.getXmmctoxmbhList(val)
      },
      // 根据项目名称查询项目编号
      getXmmctoxmbhList (param) {
        this.$api.getXmmctoxmbhList({xmmc: param}).then(res => {
          if (res.code !== 1) {
            this.$message({
              message: '网络错误',
              type: 'error'
            ...
            // this.jldModel.xmbh = res.data[0].XMBH
            this.jldModel.xmmc = res.data?res.data[0].XMMC:''
            // this.getZhList(this.jldModel.xmbh)
          }
        }).catch(e => {
          console.log(e)
        })
      ...
      // 根据项目编号查询项目名称
      getXmbhtomcArr (param) {
        // debugger
        this.$api.getXmbhtomcList({xmbh:param}).then(res => {
          ...
          if (res.code !== 1) {
            ...
            // this.jldModel.xmmc = res.data[0].XMMC
            this.jldModel.xmmc = res.data?res.data[0].XMMC:''
      ...
      // 根据项目名称和项目编号确认起止点桩号
      getZhList (param) {
        this.$api.getZhList('?xmbh='+ param).then(res => {
          if (res.status !== 200) {
            this.$message({
              message: '网络出错',
              type: 'error'
            ...
            this.qszh1 = res.data.data[0].QSZH
            this.zzzh2 = res.data.data[0].ZZZH
      ...
      updatejobOrder (type, row) {
        this.jldModel = row
        this.updateFormDia(type, this.jldModel)
      },
      // 查看详情
      seedetail (type, row) {
        this.jldModel = row
            ...
            this.$api.insertRcyhgcSjqd(paramObj).then(res => {
              if (res.status !== 200) {
                ...
                this.$message({
                  ...
                  type: res.data.msg.indexOf('失败')== -1 ? 'success' : 'error'
                })
                this.showDetail = false
                this.queryYDJHData()
              }
      ...
      // 获取车道的枚举值
      getcdMap (mjparam) {
        this.$api.jdwhxjwxgetXjType2({mjlxbm: mjparam}).then(res => {
          if (res.code != 1) {
            this.$message({
              message: '网络出错',
              type: 'error'
            ...
            this.cdarr = res.data && res.data.length !==0 ? res.data : []
          }
        })
      },
      changeCd (val) {
        this.jldModel.cd = val
      },
      // 分页展示数据
      ...
      queryYDJHData (val) {
        ...
        // debugger
        this.$api.getRcyhgcSjqdList({xmbh: xmbh == undefined ? xmbhOfinit.XMBH : xmbh,startDate: startDate, endDate: endDate, pageNum, pageSize, daZzid}).then(res => {
