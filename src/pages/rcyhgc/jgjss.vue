...
export default {
  ...
  methods: {
   ...
   let self = this
      this.$api.getJgjssBasicData({xmbh:value.XMBH}).then(res=>{
        if(res.code == 1){
    ...
    showBhFormDia (row) {
      ...
      this.$api.getJgjssDetailed({id:row.id}).then(res=>{
    ...
    search (currentPage, pageSize) {
      ...
      //daBmid,daYhid,
      this.$api.getJgjssList2({pageNum:currentPage,pageSize,daZzid,startDate:ksrq,endDate:jsrq,xmbh}).then(resp => {
        this.loading = false
          if(resp.code == 1){
    ...
    addSubmit () {
      ...
      if(this.lmshForm.gcmc){
        this.$api.insertJgjss(this.lmshForm).then(res => {
          if (res.code === 1) {
            this.$message({
              type: 'success',
              message: '添加成功!'
    ...
    updateSubmit () {
      ...
      this.$api.updateJgjss(this.lmshForm).then(res => {
        if (res.code === 1) {
          this.$message({
            type: 'success',
            message: '编辑成功!'
    ...
    deleteData () {
      ...
      if (self.multipleSelection.length > 0) {
        self.$confirm('确定删除选中的信息吗？删除后数据无法恢复！', '', {
          ...
          self.$api.deleteJgjss({id:djbharr}).then(res => {
            if (res.code === 1) {
              self.$message({
                type: 'success',
                message: '删除成功!'
    ...
    shenhe (row, index) {
      ...
      this.$confirm('是否通过审核？', '', {
        ...
        this.$api.createWorkFlow({
          ...
          org:orgmc}).then(resp=>{
            if(resp.code == 1){
              this.$api.updateJgjssSpzt({id:row.id,spzt:"1.1"}).then(res => {
                if (res.code === 1) {
                  this.$message({
                    type: 'success',
                    message: '提交成功!'
