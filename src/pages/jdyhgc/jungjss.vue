...
<template>
  <div id="gcbgd" style="height: 100%" v-loading="loading">
    <div class="header" v-show="!showDetail">
      <span class="headerTit">骏工结算书</span>
    ...
    <div class="conter_table" v-show="!showDetail">
      ...
      <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark"
        ...
        <el-table-column type="expand">
          <template scope="scope">
            <el-table class="demo-table-expand"
              ...
              <el-table-column prop="zmnr" label="子目内容"></el-table-column>
              <el-table-column prop="dw" label="单位"></el-table-column>
              <el-table-column prop="htdj" label="合同单价"></el-table-column>
              <el-table-column prop="date" label="合同">
                <el-table-column prop="htsk" label="数量"></el-table-column>
                <el-table-column prop="htje" label="金额"></el-table-column>
              </el-table-column>
              <el-table-column prop="date" label="实际完成">
                <el-table-column prop="wcsl" label="数量"></el-table-column>
                <el-table-column prop="wcje" label="金额"></el-table-column>
              </el-table-column>
              <el-table-column prop="ljwczb" label="累计完成占合同%"></el-table-column>
            </el-table>
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" prop="jssbh" label="单据编号"  show-overflow-tooltip></el-table-column>
        ...
        <el-table-column align="center" prop="jldw" label="监理单位" show-overflow-tooltip></el-table-column>
        <el-table-column align="center" prop="gldw" label="管理单位" show-overflow-tooltip></el-table-column>
        <el-table-column align="center" prop="htje" label="合同金额" show-overflow-tooltip></el-table-column>
        <el-table-column align="center" prop="jiesje" label="结算金额" show-overflow-tooltip></el-table-column>
    ...
    <div class="tabArea" v-if="showDetail"  @close="resetForm">
      <div style="position:relative;">
        ...
        <div class="SPState">
          <el-button size="mini" v-if="!addzb" @click="addzb = true"><i class="icon iconfont icon-past"></i>添加子目</el-button>
          <el-button size="mini" v-if="addzb" @click="addzibiao"><i class="icon iconfont icon-past"></i>添加</el-button>
        </div>
      </div>
      <div class="dialog_warp">
        <ul class="zhubnr"  v-if="!addzb">
          <li>
            <span>单据编号</span>
            <p>
              <el-input size="mini" type="text" :value="lmshForm.jssbh"></el-input>
            </p>
          </li>
          <li>
            <span>工程名称</span>
          ...
          <li>
            <span>合同金额</span>
            <p>
              <el-input size="mini" type="text" v-model="lmshForm.htzje"></el-input>
            </p>
          </li>
          <li>
            <span>结算金额</span>
            <p>
              <el-input size="mini" type="text" v-model="lmshForm.jiesje"></el-input>
            </p>
          </li>
          <li>
            <span>质保金</span>
            <p>
              <el-input size="mini" type="text" v-model="lmshForm.zbj"></el-input>
            </p>
          </li>
          <li>
            <span>质量缺陷扣款</span>
            <p>
              <el-input size="mini" type="text" v-model="lmshForm.zlqxkk"></el-input>
            </p>
          </li>
          <li>
            <span>决算金额</span>
            <p>
              <el-input size="mini" type="text" v-model="lmshForm.juesje"></el-input>
            </p>
          </li>
          <li>
            <span>完成合同百分比</span>
            <p>
              <el-input size="mini" type="text" v-model="lmshForm.wcht"></el-input>
            </p>
          </li>
          <li>
            <span>决算后应支付金额</span>
            <p>
              <el-input size="mini" type="text" v-model="lmshForm.jshzf"></el-input>
            </p>
          </li>
          <li>
            <span></span>
          ...
          <li class="text_line">
            <span>工程内容简述及验收情况</span>
            <p>
              <el-input class="gjzjje" size="mini" type="text" v-model="lmshForm.gcnr"></el-input>
            </p>
          </li>
          <li class="text_line">
            <span>验收意见</span>
            <p>
              <el-input class="gjzjje" size="mini" type="text" v-model="lmshForm.ysyj"></el-input>
            </p>
          </li>
          <li class="text_line">
            <span>附件</span>
      ...
      <!--    <li class="text_line">
            <span>子目</span>
            <p>
              <span style="margin-right: ;" v-for="(item, index) in lmshForm.mx" :key="index" v-if="item.gcjlmx_zmmc">{{(index+1)+'、'+item.gcjlmx_zmmc}}</span>
            </p>
          </li>-->
          <li class="text_line">
            <span>备注</span>
            <p>
              <el-input class="gjzjje"  size="mini" type="text" v-model="lmshForm.remark"></el-input>
            </p>
          </li>
          <li>
            <span>编制单位</span>
        ...
        <div class="zibnr2" v-if="addzb" style="overflow-x: auto;overflow-y: hidden;">
          <ul>
            ...
            <li v-for="(item, index) in lmshForm.mx" :key="index">
              ...
              <p>
                <el-input size="mini" type="text" v-model="item.zmnr"></el-input>
              </p>
              <p>
              ...
              <p class="tow_linep">
                <el-input size="mini" type="text" v-model="item.htsl"></el-input>
                <el-input size="mini" type="text" v-model="item.htje"></el-input>
              </p>
              <p class="tow_linep">
                <el-input size="mini" type="text" v-model="item.wcsl"></el-input>
                <el-input size="mini" type="text" v-model="item.wcje"></el-input>
              </p>
              <p>
                <el-input size="mini" type="text" v-model="item.ljwczb"></el-input>
              </p>
            </li>
            <div style="clear: both"></div>
          </ul>
...
<!--    <el-dialog class="data-dialog el-updata-div" title="竣工结算书" :close-on-click-modal="false" :visible.sync="showDetail" width="580px" @close="resetForm">
      <div class="dialog_warp">
        <ul class="zhubnr">
          <li>
            <span>单据编号</span>
          ...
          <li class="text_line">
            <span>工程内容简述及验收情况</span>
            <p>
              <el-input size="mini" type="text" v-model="lmshForm.gcnr"></el-input>
            </p>
          </li>
          <li class="text_line">
            <span>验收意见</span>
            <p>
              <el-input size="mini" type="text" v-model="lmshForm.ysyj"></el-input>
            </p>
          </li>
          <li class="text_line">
            <span>附件</span>
          ...
          <li class="text_line">
            <span>备注</span>
            <p>
              <el-input size="mini" type="text" v-model="lmshForm.remark"></el-input>
            </p>
          </li>
          <li>
            <span>编制单位</span>
...
export default {
  ...
  methods: {
    ...
    search (currentPage, pageSize) {
      ...
      this.$api.getJungList('?pageNum=' + currentPage + '&pageSize=' + pageSize + '&' + $.param(this.searchForm)).then(resp => {
        this.loading = false
        if (resp.status !== 200) {
          this.$message({
            message: '出错了',
    ...
    addSubmit () {
      this.$api.addJungForGcys(this.lmshForm).then(res => {
        if (res.data.code === 1) {
          this.$message({
            type: 'success',
            message: '添加成功!'
    ...
    updateSubmit () {
      this.$api.editJungForGcys(this.lmshForm).then(res => {
        if (res.data.code === 1) {
          this.$message({
            type: 'success',
            message: '编辑成功!'
      ...
      if (this.multipleSelection.length > 0) {
        this.$confirm('确定删除选中的信息吗？删除后数据无法恢复！', '', {
          ...
          this.multipleSelection.map((item, index) => {
            djbharr = djbharr + item.jssbh + ','
          })
          djbharr = djbharr.slice(0, djbharr.length - 1)
          this.$api.delJungForGcys('?jssbhs=' + djbharr).then(res => {
            if (res.data.code === 1) {
              this.$message({
                type: 'success',
                message: '删除成功!'
    ...
    shenhe (row, index) {
      this.$confirm('是否通过审核？', '', {
        ...
        this.$api.spJungForGcys('?spzt=已下发&jssbh=' + form.jssbh).then(res => {
          if (res.data.code === 1) {
            this.$message({
              type: 'success',
              message: '审核通过!'
  ...
  mounted () {
    this.$api.getHtJungInfoByBm('').then(res => {
