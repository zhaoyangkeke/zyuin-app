    ...
    <div class="tabArea" v-if="showAdd">
      ...
      <div class="topAdd" v-show="!showAddItem">
         <el-table  :data="addData" tooltip-effect="dark"
                   style="width: calc(100% - 26px);margin:20px auto;height: 300px;overflow-y: auto"
                   border>
          <el-table-column
            type="index"
            label="序号"
          ...
          <el-table-column align="left" label="工程考核情况">
            <template slot-scope="scope">
              <div  class="itemS">
                <input style="border: 1px solid rgb(169, 169, 169);  width: 100%;" v-model="addData[scope.$index].khqk">
              </div>
            </template>
          </el-table-column>
          <el-table-column align="left"  label="罚金(元)"   show-overflow-tooltip>
            <template slot-scope="scope">
              <input style="border: 1px solid rgb(169, 169, 169);  width: 100%;" v-model="addData[scope.$index].fj">
            </template>
          </el-table-column>
        </el-table>
      </div>
...
<script>
    ...
    data () {
      return {
        ...
        tableTits:{htbh:'项目编号',jcrq:'检查日期',htmc:'项目名称',sgdw:'施工单位',
          gldw:'管理单位',jcr:'检查人',htje:'合同金额(元)',fjhj:'违约金合计(元)',bz:'备注'},
        currentMonthData:[],
        addData:[],
        editData:[],
        editzynr:{},
    ...
    methods: {
      ...
      saveAdd(){
        ...
        if(!showAddItem){//添加第一次
          ...
          let arr = []
          let randomID = 'LYJC'+ Math.ceil(Math.random()*100000000)
          this.addData.forEach(item=>{
          ...
          this.$api.jdaddLyjc(params).then(res=>{
            if(res.code == 1){
              this.$message({
                message: '添加信息成功！',
                type: 'success'
      ...
      saveEdit(){
        if(!this.showAddItem){
          ...
          this.$api.jdupdateLyjc(editParams).then(res=>{
            if(res.code == 1){
              this.$message({
                message: '更新信息成功!',
                type: 'success'
        ...
        if(bol){
          ...
          this.$confirm('此操作将永久删除该文件, 是否继续?',  {
            ...
            if(this.delIds.length){
              this.$api.jddeleteLyjc(obj).then(res=>{
                if(res.code == 1){
                  this.$message({
                    message: '删除信息成功！',
                    type: 'success'
 ...
 /*         this.$api.jdgetLyjcbmxByMid({id:row.id}).then(res=>{
            if(res.code == 1){
              let arr = res.data
              arr.map(item =>{
    ...
    /*      this.$api.jdgetLyjcbmxByMid({id:row.id}).then(res=>{
      ...
      queryYDJHData(){
        ...
        this.loading = true
        let self = this
        this.$api.jdgetLyjcAll({
          htbh:xmbh?xmbh.trim():undefined, ksrq, jsrq,
          pageNum,pageSize,
          bmid: self.userInfo.bmid,orgid:self.userInfo.orgid,yhid:self.userInfo.yhid
        }).then(res=>{
          this.loading = false
          if(res.code == 1){
