  ...
  <div id="gcjld" style="height: 100%" v-loading="loading">
    ...
    <div class="conter_table" v-if="!showDetail">
      ...
      <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark"
        ...
        <el-table-column type="expand">
          <template scope="scope">
            <el-table class="demo-table-expand"
                      :data="scope.row.mx"
                      border>
              <el-table-column prop="gcjlmx_zmh" label="细目号"></el-table-column>
              <el-table-column prop="gcjlmx_zmmc" label="细目名称"></el-table-column>
              <el-table-column prop="gcjlmx_jldw" label="单位"></el-table-column>
              <el-table-column prop="gcjlmx_htsl" label="数量"></el-table-column>
              <el-table-column prop="gcjlmx_htje" label="金额"></el-table-column>
            </el-table>
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" prop="gcjl_yd" label="月度" show-overflow-tooltip></el-table-column>
        <el-table-column align="center" prop="gcjl_htmc" label="合同名称" show-overflow-tooltip></el-table-column>
        <el-table-column align="center" prop="gcjl_cbdw" label="承包单位" show-overflow-tooltip></el-table-column>
        <el-table-column align="center" prop="gcjl_qdzh" label="起点桩号" show-overflow-tooltip></el-table-column>
        <el-table-column align="center" prop="gcjl_zdzh" label="止点桩号" show-overflow-tooltip></el-table-column>
      </el-table>
    </div>
      ...
      <div class="dialog_warp2">
        <ul class="zhubnr" v-if="!addzb">
          <li>
            <span>单据编号</span>
            <p>
              <el-input size="mini" :disabled="showxq" type="text" :value="lmshForm.gcjl_djbh"></el-input>
            </p>
          </li>
          <li>
            <span>工程名称</span>
            <p>
              <el-select clearable :disabled="showxq" size="mini" style="width: 100%;" v-model="htbh" value-key="gcjl_htbh" placeholder="" @change="selectHt">
          ...
          <li class="text_line">
            <span>计算式及简图说明</span>
            <p>
              <el-input class="gjzjje" size="mini" :disabled="showxq" type="text" v-model="lmshForm.gcjl_jtsm"></el-input>
            </p>
          </li>
          <li class="text_line">
            <span>附件</span>
          ...
          <li class="text_line">
            <span>子目</span>
            <p>
            <span style=" " v-for="(item, index) in lmshForm.mx" :key="index" v-if="item.gcjlmx_zmmc">{{(index+1)+'、'+item.gcjlmx_zmmc}}</span>
            </p>
          </li>
          <li>
            <span>填报单位</span>
        ...
        <ul class="zhubnr">
          ...
          <li>
            ...
            <p>
              ...
              &lt;!&ndash;<el-input size="mini" type="text" v-model="lmshForm.gcjl_gcmc"></el-input>&ndash;&gt;
            </p>
          </li>
          <li>
            <span>申请日期</span>
        ...
        <div class="zibnr" v-if="addzb" style="overflow-x: auto;overflow-y: hidden;">
          ...
          <ul v-if="tabindex===0">
            ...
            <li v-for="(item, index) in lmshForm.mx" :key="index">
              ...
              <p>
                <el-input size="mini" type="text" v-model="item.gcjlmx_htje"></el-input>
                &lt;!&ndash;{{item.gcjlmx_htsl*item.gcjlmx_htdj}}&ndash;&gt;
              </p>
              <p>
                <el-input size="mini" type="text" v-model="item.gcjlmx_kkje"></el-input>
              </p>
...
export default {
  ...
  methods: {
    ...
    tongjije (item, index) {
      // this.lmshForm.mx[index].jrgmx_je = item.jrgmx_sl * item.jrgmx_dj
    },
    showAdd () {
    ...
    search (currentPage, pageSize) {
      ...
      this.$api.gczfgetGcjldList('?pageNum=' + currentPage + '&pageSize=' + pageSize + '&' + $.param(this.searchForm)).then(resp => {
        this.loading = false
        if (resp.status !== 200) {
          this.$message({
            message: '出错了',
    ...
    addSubmit () {
      ...
      this.$api.addGcjldForGczf(this.lmshForm).then(res => {
        if (res.data.code === 1) {
          this.$message({
            type: 'success',
            message: '添加成功!'
    ...
    updateSubmit () {
      this.handleRemove(this.lmshForm.djbh)
      this.$api.editGcjldForGczf(this.lmshForm).then(res => {
        if (res.data.code === 1) {
          this.$message({
            type: 'success',
            message: '编辑成功!'
    ...
    deleteData () {
      if (this.multipleSelection.length > 0) {
        this.$confirm('确定删除选中的信息吗？删除后数据无法恢复！', '', {
          ...
          this.$api.delGcjldForGczf('?djbhs=' + djbharr).then(res => {
            if (res.data.code === 1) {
              this.$message({
                type: 'success',
                message: '删除成功!'
    ...
    shenhe (row, index) {
      this.$confirm('是否通过审核？', '', {
        ...
        form.gcjl_spzt = '已下发'
        this.$api.spGcjldForGczf('?spzt=' + form.gcjl_spzt + '&djbhs=' + form.gcjl_djbh).then(res => {
          if (res.data.code === 1) {
            this.$message({
              type: 'success',
              message: '审核通过!'
    ...
    selectHt (value) {
      ...
      this.lmshForm.gcjl_cbdw = value.gcjl_cbdw
  ...
  mounted () {
    this.$api.gczfgetHtXxInfoByBm('').then(res => {
