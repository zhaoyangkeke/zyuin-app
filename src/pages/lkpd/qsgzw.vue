...
<template>
  <div id="lkpdlmsh" class="styleSheets" style="height: 100%" v-loading.fullscreen.lock="fullscreenLoading">
    <div class="bill-header">
      <div class="bill-title">
        <span>桥隧构造物</span>
        <ul>
           <li  class="Hight_btn"><a href="http://61.141.223.10:8002/lkpd/dcmb/getBcidcList" class="el-icon-download"> 模版下载</a></li>
          <li @click="chaxBtn"><i class="el-icon-search"> 查询</i></li>
          <li @click="xzBtn"><i class="el-icon-download"> 导入</i></li>
          <!-- <li><el-upload
  class="upload-demo"
  action="http://61.141.223.10:8002/lkpd/cx/importBci"
  ...
  <span>导入</span>
          ...
          <el-form :inline="true" :model="xzForm" ref="xzForm" class="xzForm">
           <el-form-item style="margin-bottom: 0px" label="" prop="" class="">
              <!-- <el-select clearable size="mini" style="width: 150px;" v-model="bbxx"    placeholder="版本名称" @change="changeBb">
                <el-option v-for="(item,index) in qmldbb" :value-key="index" :key="item.id" :label="item.bbmc" :value="item"></el-option>
              </el-select> -->
              <el-select  clearable size="mini" style="width: 150px;" v-model="bbxx"  placeholder="请选择" @change="changeBb">
         <el-option v-for="(item,index) in qmldbb"   :key="index" :label="item.bbmc" :value="item"></el-option>
  </el-select>
            </el-form-item>
            <el-form-item style="margin-bottom: 0px" label="" prop="" class="">
    ...
    <div class="conter_table">
      <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark"
       ...
       <el-table-column align="left" width="150" sortable prop="qdzh" label="起点桩号" show-overflow-tooltip>
        ...
        <el-table-column align="left" width="80" sortable prop="fx" label="行车方向" show-overflow-tooltip></el-table-column>
        <el-table-column align="left" width="100" sortable prop="jccd" label="车道" show-overflow-tooltip></el-table-column>
        ...
        <!-- <el-table-column align="left" prop="dcry" label="记录人员"></el-table-column> -->
        <el-table-column align="left" width="100" sortable prop="lmkd" label="检测宽度" show-overflow-tooltip></el-table-column>
        <!-- <el-table-column align="center" label="桥梁技术状况(座)" show-overflow-tooltip>
          <el-table-column align="center" prop="qljszk1" label="一、二" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[0].ljsj }}</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="qljszk2" label="三" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[1].ljsj }}</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="qljszk3" label="四" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[2].ljsj }}</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="qljszk4" label="五" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[3].ljsj }}</p>
            </template>
          </el-table-column>
        </el-table-column>
        <el-table-column align="center" label="隧道(座)" show-overflow-tooltip>
          <el-table-column align="center" prop="sd1" label="无异常" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[4].ljsj }}</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="sd2" label="有异常" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[5].ljsj }}</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="sd3" label="有危险" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[6].ljsj }}</p>
            </template>
          </el-table-column>
        </el-table-column>
        <el-table-column align="center" label="涵洞" show-overflow-tooltip>
          <el-table-column align="center" prop="hd1" label="好、较好" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[7].ljsj }}</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="hd2" label="较差" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[8].ljsj }}</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="hd3" label="差" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[9].ljsj }}</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="hd4" label="危险" width="45" show-overflow-tooltip>
            <template scope="scope">
    ...
    <el-dialog fullscreen class="data-dialog el-updata-div"  :show-close="false" :close-on-click-modal="false" :visible.sync="showDetail" width="720px"  @close="resetForm">
      ...
      <qsgzwmb :data="lmshForm" @childByValue="childByValue"  @editOk="editOk" v-if="showDetail"></qsgzwmb>
    </el-dialog>
  </div>
</template>
...
import qsgzwmb from './mb/qsgzwmb.vue'
export default {
  components: {qsgzwmb},
  props: {
    hasMenu: {
  ...
  methods: {
    ...
    showAdd () {
      ...
      this.showxq = false
      this.isadd = true
      this.src = config.bbsrc + encodeURI('reportlet=/zsxz/桥隧构造物调查.cpt&op=write&dcry=管理员&bbid=10220&condition=and qm.lmlx=1&lxid=03&lxcode=') + this.drForm.lxcode + '&szhh=' + this.drForm.szhh + '&ezhh=' + this.drForm.ezhh + '&fx=' + this.drForm.xcfx
      // this.showdrxz = false
    },
    editOk (){
  this.showDetail=false
  ...
  this.search(this.currentPage, this.pageSize)
    ...
    updateFormDia (row, index) {
      ...
      //   dcid = row.dcid
      // }
      // this.src = config.bbsrc + 'reportlet=/zsxz/桥隧构造物调查.cpt&op=write&dcry=管理员&dcbbid=10220&lxid=03&ldid=' + row.ldid + '&dcid=' + dcid
      // this.index = index
    },
    showBhFormDia (row) {
      this.$api.getBciXiang('?dcid='+row.dcid).then(resp => {
...
this.$refs.multipleTable.toggleRowSelection(row)
    ...
    search (currentPage, pageSize) {
      ...
      this.$api.getBciAllList('?pageNum=' + currentPage + '&pageSize=' + pageSize + '&' + $.param(this.searchForm)).then(resp => {
        this.loading = false
        if (resp.status !== 200) {
          this.$message({
            message: '出错了',
    ...
    download (){
      ...
      if(this.xzForm.sjlx=='千米版'){
        this.upUrl='http://61.141.223.10:8002/lkpd/drmb/importBciList'
      }else{
        this.upUrl='http://61.141.223.10:8002/lkpd/drmb/importBciBMList'
      }
       this.$refs.uploads.submit();
      // this.$api.getBciList('?' + $.param(this.xzForm)).then(resp => {
      //   this.loading = true
      //   if (resp.status !== 200) {
      //     this.$message({
      //       message: '出错了',
      ...
      //       downloadElement.download = `桥隧构造物模板`; //下载后文件名
