...
<template>
  <div id="lkpdlmsh" class="styleSheets" style="height: 100%" v-loading.fullscreen.lock="fullscreenLoading">
    <div class="bill-header">
      ...
      <div class="bill-title">
        <span>路面车辙</span>
        <ul>
          <li  class="Hight_btn"><a href="http://61.141.223.10:8002/lkpd/dcmb/getRdidcList" class="el-icon-download"> 模版下载</a></li>
          <li @click="chaxBtn"><i class="el-icon-search"> 查询</i></li>
          <li @click="xzBtn"><i class="el-icon-download"> 导入</i></li>
         <!-- <li><el-upload
  class="upload-demo"
  action="http://61.141.223.10:8002/lkpd/cx/importRdi"
  ...
  <span>导入</span>
    ...
    <div class="conter_table">
      <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark"
       ...
       <el-table-column align="left" width="150" sortable prop="qdzh" label="起点桩号" show-overflow-tooltip>
        ...
        <el-table-column align="left" width="100" sortable prop="jcrq" label="检测日期"></el-table-column>
        <el-table-column align="left" prop="jcsb" sortable label="检测设备"></el-table-column>
        <el-table-column align="left" prop="czsd" sortable label="车辙(cm)"></el-table-column>
        <!-- <el-table-column align="left" prop="WDCZ" label="外道车辙(cm)"></el-table-column> -->
        <el-table-column align="left" prop="jlry" sortable label="记录人员"></el-table-column>
    ...
    <el-dialog fullscreen class="data-dialog el-updata-div"  :show-close="false" :close-on-click-modal="false" :visible.sync="showDetail" width="720px"  @close="resetForm">
      ...
      <!-- <div style="width: 685px;height: 600px;margin:0 auto">
        <iframe :src="src" frameborder="0" style="width: calc(100%);height: 100%;margin:0 auto;"></iframe>
      </div> -->

      <lmczmb :data="formData" @childByValue="childByValue"  @editOk="editOk" v-if="showDetail"></lmczmb>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    showAdd () {
      ...
      this.showxq = false
      this.isadd = true
      this.src = config.bbsrc + encodeURI('reportlet=/zsxz/lmjcRDI.cpt&op=write&dcry=管理员&bbid=10220&type=0402&lxcode=') + this.drForm.lxcode + '&szhh=' + this.drForm.szhh + '&ezhh=' + this.drForm.ezhh + '&fx=' + this.drForm.xcfx
      // this.showdrxz = false
    },
    updateFormDia (row, index) {
      this.formData = JSON.parse(JSON.stringify(row))
      this.formData.isxq=false
      this.showDetail = true
...
this.$refs.multipleTable.toggleRowSelection(row)
  ...
  this.search(this.currentPage, this.pageSize)
    ...
    search (currentPage, pageSize) {
      ...
      this.$api.getRdiAllList('?pageNum=' + currentPage + '&pageSize=' + pageSize + '&' + $.param(this.searchForm)).then(resp => {
        this.loading = false
        if (resp.status !== 200) {
          this.$message({
            message: '出错了',
    ...
    download (){
      ...
      if(this.xzForm.sjlx=='千米版'){
        this.upUrl='http://61.141.223.10:8002/lkpd/drmb/importRdiList'
      }else{
        this.upUrl='http://61.141.223.10:8002/lkpd/drmb/importRdiBMList'
      }
      this.$refs.uploads.submit();
      // this.$api.getRdiList('?' + $.param(this.xzForm)).then(resp => {
      //   this.loading = true
      //   if (resp.status !== 200) {
      //     this.$message({
      //       message: '出错了',
      ...
      //       downloadElement.download = `路面车辙模板`; //下载后文件名
