...
<template>
  <div id="lkpdlmsh" class="styleSheets" style="height: 100%"  v-loading.fullscreen.lock="fullscreenLoading">
    <div class="bill-header">
      <div class="bill-title">
        <span>结构强度</span>
        <!-- <ul>
          <li><a href="./static/jgqd.xlsx"><i class="el-icon-download"> 模版下载</i></a></li>

        </ul> --> 
        <ul>
          <li  class="Hight_btn"><a href="http://61.141.223.10:8002/lkpd/dcmb/getPssidcList" class="el-icon-download"> 模版下载</a></li>
          <li @click="chaxBtn"><i class="el-icon-search"> 查询</i></li>
          <li @click="xzBtn"><i class="el-icon-download"> 导入</i></li>
           <!-- <li><el-upload
  class="upload-demo"
  action="http://61.141.223.10:8002/lkpd/cx/importPssi"
  ...
  <span>导入</span>
      ...
      <div class="drxz bill-search" v-if="showdrxz">
          ...
          <el-form-item style="margin-bottom: 0px" label="" prop="" class="">
              <el-input clearable size="mini" type="text" v-model="xzForm.jcsb"  placeholder="请输入检测设备"></el-input>
          </el-form-item>
          <el-form-item style="margin-bottom: 0px" label="" prop="" class="">
    ...
    <div class="conter_table">
      <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark"
        ...
        <el-table-column align="left" sortable prop="lxbm" label="路线编号"  show-overflow-tooltip></el-table-column>
       <el-table-column align="center" prop="lxmc" label="路线名称" width="80" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column align="left" sortable prop="fx" label="行车方向"></el-table-column>
        <el-table-column sortable align="left" prop="jccd" label="车道"></el-table-column>
        <el-table-column align="left" sortable prop="jcrq" label="检测日期"></el-table-column>
        <el-table-column sortable align="left" prop="jcsb" label="检测设备"></el-table-column>
        <!-- <el-table-column sortable align="left" prop="SCDBWC" label="实测代表弯沉"></el-table-column> -->
        <el-table-column sortable align="left" prop="ssr" label="路面结构强度系数"></el-table-column>
        <el-table-column sortable align="left" prop="jlry" label="记录人员"></el-table-column>
        <el-table-column align="center" class-name="column-caoz" label="操作" width="80">
          <template scope="scope">
            <span style="color:#00a2fd;cursor: pointer;text-align: center" @click="updateFormDia(scope.row, scope.$index)">编辑</span>
            <span style="color:#00a2fd;cursor: pointer;text-align: center" @click="showBhFormDia(scope.row)">详情</span>
    ...
    <el-dialog fullscreen class="data-dialog el-updata-div" title="" :close-on-click-modal="false" :visible.sync="showDetail" width="720px"  :show-close="false">
      <span slot="title">
...
import ljmb from './mb/jgqdmb.vue'
export default {
  components: {ljmb},
  props: {
    hasMenu: {
  ...
  this.search(this.currentPage, this.pageSize)
    ...
    showAdd () {
      ...
      this.showxq = false
      this.isadd = true
      this.src = config.bbsrc + encodeURI('reportlet=/zsxz/lmjcPSSI.cpt&op=write&dcry=管理员&bbid=10220&type=0404&lxcode=') + this.drForm.lxcode + '&szhh=' + this.drForm.szhh + '&ezhh=' + this.drForm.ezhh + '&fx=' + this.drForm.xcfx
      // this.showdrxz = false
    },
   ...
   this.lmshForm = JSON.parse(JSON.stringify(row))
    ...
    search (currentPage, pageSize) {
      this.loading = true
      this.$api.getPssiAllList('?pageNum=' + currentPage + '&pageSize=' + pageSize + '&' + $.param(this.searchForm)).then(resp => {
        this.loading = false
        if (resp.status !== 200) {
          this.$message({
            message: '出错了',
          ...
          for(let i=0;i< this.tableData.length;i++){
            if(this.tableData[i].jcrq=='' || this.tableData[i].jcrq==undefined){
    ...
    download (){
      ...
      if(this.xzForm.sjlx=='千米版'){
        this.upUrl='http://61.141.223.10:8002/lkpd/drmb/importPssiList'
      }else{
        this.upUrl='http://61.141.223.10:8002/lkpd/drmb/importPssiBMList'
      }
      this.$refs.uploads.submit();
    //   this.$api.getPssiList('?' + $.param(this.xzForm)).then(resp => {
    //     this.loading = true
    //     if (resp.status !== 200) {
    //       this.$message({
    //         message: '出错了',
    ...
    //         downloadElement.download = `结构强度模板`; //下载后文件名
   ...
   this.openFullScreen()
    ...
    form.append("jcsb", this.xzForm.jcsb);
