  ...
  <div id="lkpdlmsh" class="styleSheets" style="height: 100%" v-loading.fullscreen.lock="fullscreenLoading">
    <div class="bill-header">
      ...
      <div class="bill-title">
        <span>路基</span>
        <ul>
           <li  class="Hight_btn"><a href="http://61.141.223.10:8002/lkpd/dcmb/getScidcList" class="el-icon-download"> 模版下载</a></li>
          <li @click="chaxBtn"><i class="el-icon-search"> 查询</i></li>
          <li @click="xzBtn"><i class="el-icon-download"> 导入</i></li>
          <!-- <li><el-upload
  class="upload-demo"
  action="http://61.141.223.10:8002/lkpd/cx/importSci"
  ...
  <span>导入</span>
     ...
     <div class="drxz bill-search" v-if="showdrxz">
      ...
      <div class="bill-search" style="" v-if="showSearch">
        <el-form :inline="true" :model="searchForm" ref="searchForm" class="searchForm">
          <el-form-item style="margin-bottom: 0px" label="" prop="" class="">
            <el-select clearable size="mini" style="width: 150px " v-model="lxData" value-key="id" placeholder="请选择路线" @change="changelx">
              <el-option v-for="(item,index) in qmbbOptions" :key="index" :label="item.ldname" :value="item"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item style="margin-bottom: 0px" label="" prop="" class="">
    ...
    <div class="conter_table">
      <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark"
        ...
        <el-table-column  align="left" width="100" sortable prop="lxbm" label="路线编号"  show-overflow-tooltip></el-table-column>
        <el-table-column align="left" prop="lxmc" label="路线名称"  show-overflow-tooltip></el-table-column>
        ...
        <el-table-column align="left" sortable width="100" prop="jccd" label="车道"></el-table-column>
        ...
        <el-table-column align="left" sortable prop="dcry" label="记录人员"></el-table-column>
        <el-table-column align="left" sortable width="100" prop="lmkd" label="检测宽度"></el-table-column>
        <!-- <el-table-column align="center" prop="ljbgbj" label="路基边沟不洁">
          <template scope="scope">
            <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[0].ljsj }}</p>
          </template>
        </el-table-column>
        <el-table-column align="center"  label="露肩损坏" show-overflow-tooltip>
          <el-table-column align="center" prop="ljsh1" label="轻" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[1].ljsj }}</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="ljsh2" label="重" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[2].ljsj }}</p>
            </template>
          </el-table-column>
        </el-table-column>
        <el-table-column align="center" label="边坡坍塌" show-overflow-tooltip>
          <el-table-column align="center" prop="bptt1" label="轻" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[3].ljsj }}</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="bptt2" label="中" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[4].ljsj }}</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="bptt3" label="重" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[5].ljsj }}</p>
            </template>
          </el-table-column>
        </el-table-column>
        <el-table-column align="center" label="水毁冲沟" show-overflow-tooltip>
          <el-table-column align="center" prop="shcg1" label="轻" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[6].ljsj }}</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="shcg2" label="中" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[7].ljsj }}</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="shcg3" label="重" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[8].ljsj }}</p>
            </template>
          </el-table-column>
        </el-table-column>
        <el-table-column align="center" label="路基构造物" show-overflow-tooltip>
          <el-table-column align="center" prop="ljgzw1" label="轻" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[9].ljsj }}</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="ljgzw2" label="中" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[10].ljsj }}</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="ljgzw3" label="重" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[11].ljsj }}</p>
            </template>
          </el-table-column>
        </el-table-column>
        <el-table-column align="center" prop="lyssh" label="路缘石损坏" show-overflow-tooltip>
          <template scope="scope">
            <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[12].ljsj }}</p>
          </template>
        </el-table-column>
        <el-table-column align="center" label="路基沉降" show-overflow-tooltip>
          <el-table-column align="center" prop="ljcj1" label="轻" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[13].ljsj }}</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="ljcj2" label="中" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[14].ljsj }}</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="ljcj3" label="重" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[15].ljsj }}</p>
            </template>
          </el-table-column>
        </el-table-column>
        <el-table-column align="center" label="排水系统淤塞" show-overflow-tooltip>
          <el-table-column align="center" prop="psxtys1" label="轻" width="45" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[16].ljsj }}</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="psxtys2" label="重" width="45" show-overflow-tooltip>
            <template scope="scope">
    ...
    <el-dialog fullscreen class="data-dialog el-updata-div"  :show-close="false" :close-on-click-modal="false" :visible.sync="showDetail" width="720px"  >
      <span slot="title">
     ...
     <!-- <div style="width: 100%;height: 600px;">
      ...
      <ljmb :data="lmshForm" @childByValue="childByValue"  @editOk="editOk" v-if="showDetail"></ljmb>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    showAdd () {
      ...
      this.showxq = false
      this.isadd = true
      this.src = config.bbsrc + encodeURI('reportlet=/zsxz/路基调查.cpt&op=write&dcry=管理员&bbid=10220&condition=and qm.lmlx=1&lxid=02&lxcode=' + this.drForm.lxcode + '&szhh=' + this.drForm.szhh + '&ezhh=' + this.drForm.ezhh + '&fx=' + this.drForm.xcfx)
      // this.showdrxz = false
    },
    updateFormDia (row, index) {
      ...
      //   dcid = row.dcid
      // }
      // this.src = config.bbsrc + encodeURI('reportlet=/zsxz/lkdc.cpt&op=write&dcry=管理员&dcbbid=10220&lxid=02&ldid=' + row.ldid + '&dcid=' + dcid)
      // this.index = index
    },
    editOk (){
  this.showDetail=false
...
this.$refs.multipleTable.toggleRowSelection(row)
    ...
    search (currentPage, pageSize) {
      ...
      this.$api.getSciAllList('?pageNum=' + currentPage + '&pageSize=' + pageSize + '&' + $.param(this.searchForm)).then(resp => {
        this.loading = false
        if (resp.status !== 200) {
          this.$message({
            message: '出错了',
    ...
    download (){
     ...
     if(!this.xzForm.sjlx){
      ...
      if(this.xzForm.sjlx=='千米版'){
        this.upUrl='http://61.141.223.10:8002/lkpd/drmb/importSciList'
      }else{
        this.upUrl='http://61.141.223.10:8002/lkpd/drmb/importSciBMList'
      }
       this.$refs.uploads.submit();
    //   this.$api.getSciList('?' + $.param(this.xzForm)).then(resp => {
    //     this.loading = true
    //     if (resp.status !== 200) {
    //       this.$message({
    //         message: '出错了',
    ...
    //         downloadElement.download = `路基模板`; //下载后文件名
