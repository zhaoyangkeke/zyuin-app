  ...
  <div id="lkpdlmsh" class="styleSheets" style="height: 100%" v-loading.fullscreen.lock="fullscreenLoading">
    <div class="bill-header">
      <div class="bill-title">
        <span>路面平整度</span>
        <ul>
         <li  class="Hight_btn"><a href="http://61.141.223.10:8002/lkpd/dcmb/getRqidcList" class="el-icon-download"> 模版下载</a></li>
          <li @click="chaxBtn"><i class="el-icon-search"> 查询</i></li>
          <li @click="xzBtn"><i class="el-icon-download"> 导入</i></li>
          <!-- <li><el-upload
  class="upload-demo"
  action="http://61.141.223.10:8002/lkpd/cx/importRqi"
  ...
  <span>导入</span>
    ...
    <div class="conter_table">
      <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark"
        ...
        <el-table-column align="left" prop="iri" sortable label="IRI"></el-table-column>
    ...
    <!--<el-dialog fullscreen class="data-dialog el-updata-div" title="添加" :close-on-click-modal="false" :visible.sync="showDetail" width="580px" @close="resetForm">
    ...
    <el-dialog fullscreen class="data-dialog el-updata-div" :show-close="false" :close-on-click-modal="false" :visible.sync="showDetail" width="720px"  @close="resetForm">
      ...
      <lmpzdmb :data="formData" @childByValue="childByValue"  @editOk="editOk" v-if="showDetail"></lmpzdmb>
    </el-dialog>
  </div>
</template>
...
import lmpzdmb from './mb/lmpzdmb.vue'
...
export default {
  ...
  methods: {
    ...
    showAdd () {
      ...
      this.showxq = false
      this.isadd = true
      this.src = config.bbsrc + encodeURI('reportlet=/zsxz/lmjcRQI.cpt&op=write&dcry=管理员&bbid=10220&type=0401&lxcode=') + this.drForm.lxcode + '&szhh=' + this.drForm.szhh + '&ezhh=' + this.drForm.ezhh + '&fx=' + this.drForm.xcfx
      // this.showdrxz = false
    },
...
this.$refs.multipleTable.toggleRowSelection(row)
    ...
    search (currentPage, pageSize) {
      ...
      this.$api.getRqiAllList('?pageNum=' + currentPage + '&pageSize=' + pageSize + '&' + $.param(this.searchForm)).then(resp => {
        this.loading = false
        if (resp.status !== 200) {
          this.$message({
            message: '出错了',
  ...
  this.search(this.currentPage, this.pageSize)
    ...
    download (){
      ...
      if(this.xzForm.sjlx=='千米版'){
        this.upUrl='http://61.141.223.10:8002/lkpd/drmb/importRqiList'
      }else{
        this.upUrl='http://61.141.223.10:8002/lkpd/drmb/importRqiBMList'
      }
      this.$refs.uploads.submit();
      // this.$api.getRqiList('?' + $.param(this.xzForm)).then(resp => {
      //   this.loading = true
      //   if (resp.status !== 200) {
      //     this.$message({
      //       message: '出错了',
      ...
      //       downloadElement.download = `路面平整度模板`; //下载后文件名
