  ...
  <div id="lkpdlmsh" class="styleSheets" style="height: 100%" v-loading.fullscreen.lock="fullscreenLoading">
    <div class="bill-header">
      <div class="bill-title">
        <span>路面磨耗</span>
        <ul>
            <li  class="Hight_btn"><a href="http://61.141.223.10:8002/lkpd/dcmb/getPwidcList" class="el-icon-download"> 模版下载</a></li>
          <li @click="chaxBtn"><i class="el-icon-search"> 查询</i></li>
          <li @click="xzBtn"><i class="el-icon-download"> 导入</i></li>
          <!-- <li><el-upload
  class="upload-demo"
  action="http://61.141.223.10:8002/lkpd/cx/importPwi"
  ...
  <span>导入</span>
    ...
    <div class="conter_table">
      ...
      <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark"
        ...
        <!-- <el-table-column align="center"  label="路面构造深度(mm)" show-overflow-tooltip>
          <el-table-column align="center"  label="基准值" width="60" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[0].ljsj }}</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="jl2" label="左轮深度" width="70" show-overflow-tooltip>
            <template scope="scope">
              <p v-if="scope.row.lkdcfb.length>0">{{ scope.row.lkdcfb[1].ljsj }}</p>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="jl3" label="右轮深度" width="70" show-overflow-tooltip>
            <template scope="scope">
    ...
    <el-dialog fullscreen class="data-dialog el-updata-div" :show-close="false" :close-on-click-modal="false" :visible.sync="showDetail" width="720px"  @close="resetForm">
      ...
      <lmmhmb :data="lmshForm" @childByValue="childByValue"  @editOk="editOk" v-if="showDetail"></lmmhmb>
    </el-dialog>
  </div>
</template>
...
import lmmhmb from './mb/lmmhmb.vue'
export default {
  components: {lmmhmb},
  props: {
    hasMenu: {
  ...
  this.search(this.currentPage, this.pageSize)
    ...
    updateFormDia (row, index) {
      this.$api.getPwiXiang('?dcid='+row.dcid).then(resp => {
        if (resp.status !== 200) {
          this.$message({
            message: '出错了',
            type: 'error'
...
this.$refs.multipleTable.toggleRowSelection(row)
    ...
    search (currentPage, pageSize) {
      ...
      this.$api.getPwiAllList('?pageNum=' + currentPage + '&pageSize=' + pageSize + '&' + $.param(this.searchForm)).then(resp => {
        this.loading = false
        if (resp.status !== 200) {
          this.$message({
            message: '出错了',
    ...
    download (){
      ...
      if(this.xzForm.sjlx=='千米版'){
        this.upUrl='http://61.141.223.10:8002/lkpd/drmb/importPwiList'
      }else{
        this.upUrl='http://61.141.223.10:8002/lkpd/drmb/importPwiBMList'
      }
      this.$refs.uploads.submit();
      // this.$api.getPwiList('?' + $.param(this.xzForm)).then(resp => {
      //   this.loading = true
      //   if (resp.status !== 200) {
      //     this.$message({
      //       message: '出错了',
      ...
      //       downloadElement.download = `路面磨耗模板`; //下载后文件名
