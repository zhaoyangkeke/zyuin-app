<template>
    <el-dialog
      ...
      <div>
        ...
        <el-table :data="dialogData" @row-click="rowClick" :row-class-name="tableRowClassName" :row-style="selectedHighlight" @row-dblclick="hh" class="aaa" max-height="300" style="width: 100%;">
          <el-table-column
            class="aaaa"
            type="index"
            align="center"
...
<script>
  /*8.1日选中要添加的项目后调用selectAddDialog方法，现在去掉，不会在当前页面去调用*/
  export default {
      ...
      hh(row, event){
        ...
        switch (self.isAdd){
          case 'ty':
            ...
            break;
          case 'gys':
            self.$emit('closegysMask',row)
            break;
          case 'htInfo':
            self.$emit('closeMask',row)
            break;
          case 'add':
      ...
      searchBtn(){
        ...
        switch (this.datatypes){
          ...
          case 'bh':
            ...
            break;
          case 'ht':
            this.$emit('searchHTinfo',searchVal.trim())
            break;
          case 'lxbh':
      ...
      conformClose(type){
        ...
        switch (self.isAdd){
          case 'ty':
            ...
            break;
          case 'gys':
            type == 'cancel'?self.$emit('closegysMask',null): self.$emit('closegysMask',this.selectSearch)
            break;
          case 'htInfo':
            type == 'cancel'?self.$emit('closeMask',null): self.$emit('closeMask',this.selectSearch)
            break;
          case 'add':
